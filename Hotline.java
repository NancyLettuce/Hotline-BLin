...
public class Hotline {
    ...
    public boolean battle(Monster generic) {
		...
		//System.out.println(generic.isAlive() && player.isAlive());
		//System.out.println(generic.HP);
				...
				try {
				    ...
				    if (choice == 1) {//fight
						...
						if (checkAnswer(teach) == false) {
							...
							teach.HP = -200;
							System.out.println("\nYou are Correct!\nThe horrific Teacher begins to smoke around the edges.");
    	...
    	while(floors.size() > 0) {
			...
			if (floor == 2) {//if math
				...
				if (battle(teach)==false) {
				    ...
				    System.out.println(teach.drake);
				}
				encounter --;
			    }
			    encounter = 1;//reset
